...
from incremental_attn import IncrementalAttention
import copy
...
class IncrementalModel(RobertaModel):
    ...
    def __init__(self, config, add_pooling_layer=False):
        ...
        for i, layer in enumerate(self.encoder2assist.layer):
            layer.attention.self = IncrementalAttention(config)
...
class IncrementalForSequenceClassification(RobertaForSequenceClassification):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config):
        super().__init__(config)

        self.roberta = IncrementalModel(config, add_pooling_layer=True)
