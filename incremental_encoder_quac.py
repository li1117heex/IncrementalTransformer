...
from incremental_attn_quac import IncrementalSelfAttentionCross
import copy

class IncrementalLayer(RobertaLayer):
...
class IncrementalEncoder(RobertaEncoder):
    def __init__(self, config):
        super().__init__(config)
        # self.config = config
        self.layer = nn.ModuleList([IncrementalLayer(config) for _ in range(config.num_hidden_layers)])
...
class IncrementalModel(RobertaModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder2assist = IncrementalEncoder(config2assist)
        self.encoder3 = RobertaEncoder(config3)
        for i, layer in enumerate(self.encoder2assist.layer):
            layer.attention.self = IncrementalSelfAttentionCross(config)
    ...
    def get_extended_attention_mask2(self, attention_mask, input_shape, device) -> torch.Tensor:
        ...
        mask2=extended_attention_mask.transpose(2,3)==extended_attention_mask
    ...
    def get_extended_attention_mask2assist(self, attention_mask, input_shape, device) -> torch.Tensor:
        """
        Makes broadcastable attention and causal masks so that future and masked tokens are ignored.
        ...
        extended_attention_mask=extended_attention_mask.repeat(1,1,input_shape[1],1)
        mask2=extended_attention_mask.transpose(2,3)>=extended_attention_mask
        extended_attention_mask=torch.logical_and(mask2,extended_attention_mask>0).type_as(extended_attention_mask)
    ...
    def forward(
        ...
        long_input_ids=torch.cat([input_ids,question_input_ids],dim=1)

        # attention_mask2 = torch.cat([question_attention_mask*2, attention_mask], dim=1)
        long_attention_mask = torch.cat([attention_mask, (question_attention_mask>0).type_as(question_attention_mask)], dim=1)
        ...
        token_type_ids = torch.zeros(long_input_ids.size(), dtype=torch.long, device=device)
        ...
        extended_attention_mask2: torch.Tensor = self.get_extended_attention_mask2(question_attention_mask, input_shape, device)
        extended_attention_mask2assist: torch.Tensor = torch.cat([extended_attention_mask.repeat(1,1,input_shape[1],1), self.get_extended_attention_mask2assist(question_attention_mask, input_shape, device)],dim=-1)
        extended_attention_mask3: torch.Tensor = self.get_extended_attention_mask(long_attention_mask, long_input_ids.size(), device)
        ...
        embedding_output1 = embedding_output[:,:512]
        embedding_output2 = embedding_output[:,512:]
        ...
        encoder2_output = self.encoder2assist(
            encoder2_output[0],
            attention_mask=extended_attention_mask2assist,
            # head_mask=head_mask,
        ...
        encoder3_input = torch.cat([encoder_outputs.hidden_states[self.encoder2.config.num_hidden_layers + self.encoder2assist.config.num_hidden_layers], encoder2_output[0]],dim=1)
        ...
        sequence_output = self.encoder3(
            encoder3_input,
            attention_mask=extended_attention_mask3,
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_extended_attention_mask,
            past_key_values=past_key_values,
...
class IncrementalForConversationQuestionAnswering(RobertaForQuestionAnswering):
    ...
    def __init__(self, config):
        ...
        #
        self.roberta = IncrementalModel(config)
        ...
        # )
        # self.teacher = RobertaForConversationQuestionAnswering.from_pretrained(
        #     config.teacher_model_path,
    ...
    def forward(
        ...
        logits = self.qa_outputs(sequence_output[:,:512])
        ...
        if start_positions is not None and end_positions is not None:
            ...
            class_loss = class_loss_fct(class_logits.squeeze(), ((start_positions+end_positions)>0).type_as(class_logits))
            ...
            teacher_class_logits = teacher_results['attentions'].sigmoid()
            sl_loss_fct = CrossEntropyLoss()
            slc_loss_fct = BCEWithLogitsLoss()
            teach_loss = (sl_loss_fct(start_logits / self.temperature, teacher_start_logits) + sl_loss_fct(
                end_logits / self.temperature, teacher_end_logits) + slc_loss_fct(class_logits, teacher_class_logits)) / 3
...
def convert_ro_incr(model,config):
    ...
    teacher_config.max_position_embeddings = teacher_config.max_position_embeddings * 2
    teacher_config.num_labels=2
    model.teacher = RobertaForConversationQuestionAnswering.from_pretrained(
        config.teacher_model_path,
        from_tf=bool(".ckpt" in config.teacher_model_path),
        config=teacher_config,
        cache_dir=None,
    ...
    #for param in model.roberta.encoder.parameters():
        #param.requires_grad = False
    new_embeddings = nn.Embedding(config.max_position_embeddings * 2, config.hidden_size)
    new_embeddings.weight.data = model.roberta.embeddings.position_embeddings.weight.data.repeat(2, 1)
