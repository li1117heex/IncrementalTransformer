...
# from incremental_attn import IncrementalRobertaSelfAttentionCross
import copy

class QuACRobertaLayer(RobertaLayer):
...
class QuACRobertaEncoder(RobertaEncoder):
    def __init__(self, config):
        super().__init__(config)
        # self.config = config
        self.layer = nn.ModuleList([QuACRobertaLayer(config) for _ in range(config.num_hidden_layers)])
...
class QuACRobertaModel(RobertaModel):
    """
    ...
    def forward(
        ...
        long_attention_mask = torch.cat([attention_mask, question_attention_mask], dim=1)
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            input_shape = long_input_ids.size()
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(long_attention_mask, input_shape, device)
        ...
        #     past_key_values_length=past_key_values_length,
        # )
        # embedding_output = torch.concat([embedding_output1,embedding_output2],dim=1)
...
class RobertaForConversationQuestionAnswering(RobertaForQuestionAnswering):
    ...
    def __init__(self, config):
        ...
        #
        self.roberta = QuACRobertaModel(config)
    ...
    def forward(
        ...
        # ends = [i.item() for i in question_attention_mask.sum(dim=1)]
        # for i,end in enumerate(ends):
        #     temp = start_logits[i,0]
        #     start_logits[i,0] = start_logits[i,end-1]
        #     start_logits[i, end - 1] = temp
        #     temp = end_logits[i, 0]
        #     end_logits[i, 0] = end_logits[i, end - 1]
        #     end_logits[i, end - 1] = temp
