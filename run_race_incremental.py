...
from incremental_encoder_race import IncrementalForMultipleChoice,convert_ro_incr
...
def main():
    ...
    if data_args.train_file is not None or data_args.validation_file is not None:
        ...
        raw_datasets = load_from_disk(dataset_path="race")
    ...
    def preprocess_function(examples):
        first_sentences = [[re.sub(r"\s+", " ", context.replace("\n", " "))] * 4 for context in examples['article']]
        question_headers = examples['question']
        second_sentences = [
            [re.sub(r"\s+", " ", ques.replace("_", " " + op + " ")) for op in examples['options'][i]]
            if "_" in ques else [re.sub(r"\s+", " ", f"{ques} {op}") for op in examples['options'][i]]
            for i, ques in enumerate(question_headers)
        ]
        ...
        tokenized_examples["label"] = [ord(ans) - ord("A") for ans in examples["answer"]]
